...
class StyleStack(Stack):
    ...
    def _gen_lib_embeddings(self, image_paths):
        for path in image_paths:
            try:
                feature_vector = self._extract_features(path)

                if feature_vector is None:
                    print(f"❌ Feature extraction failed for: {path}")
                else:
                    print(f"✅ Feature extracted for: {path}, Shape: {feature_vector.shape}")
    ...
    def _index_vectors(self):
        if not self.vector_buffer:
            raise ValueError("❌ Vector buffer is empty! No embeddings were generated.")
        """
        ...
        for j, gram_list in enumerate(self.vector_buffer):
            ...
            print(f"🟢 Stacking {len(gram_list)} feature vectors for layer: {layer}")
